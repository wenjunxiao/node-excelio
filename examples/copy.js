...
['Test1', 'Test2', 'Test3'].map((name, i) => {
  ...
  sheet.row().cell(name + 'Title1', titleOpts).width(10)
    ...
    .cell(name + 'Title3', titleOpts).width(12).mergeCell(2);
  const data = [{
      v1: 11 + i * 1000,
      v2: 12 + i * 1000,
      v3: 13 + i * 1000
...
const filename = path.resolve(__dirname, 'copy.orig.xlsx');
writer.save(filename);
console.log('saved =>', filename);
const reader = Excel.createReader({});
reader.readFile(filename, {cellStyles: true,cellNF: true, cellDates: true});
const writer1 = Excel.createWriter({
  NaN: '-',
})
writer1.loadFromReader(reader);
const filename1 = path.resolve(__dirname, 'copy.result.xlsx');
