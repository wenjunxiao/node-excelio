...
class Sheet {
  constructor(options, watermark) {
    ...
    this.width2wpx = this._px ? px : char2px;
    this.s = {
      "!row": [{
        wpx: 67
      }]
  ...
  cell (v, options, type, format) {
    ...
    if (options || options === 0) {
      ...
      let fsz = cell.s.font && cell.s.font.sz || 10;
      let width = this.width2wpx(typeof options === 'object' ? options.width : options, fsz);
      if (width === 0) {
        width = char2px(iconv.encode(v.toString(), 'gbk').length);
      }
      if (!width) {
        width = this.width2wpx(this._opts.width, fsz);
      }
      if (width) {
        if (this._opts.minWidth && width < this._opts.minWidth) {
          width = this.width2wpx(this._opts.minWidth, fsz);
        }
        let col = this['!cols'][this.colIdx];
        if (!col) {
          col = this['!cols'][this.colIdx] = {};
        }
        if (!(col.wpx && col.wpx > width)) {
          col.wpx = width;
        }
      } else if (v) {
        if (!this['!cols'][this.colIdx]) {
          this['!cols'][this.colIdx] = {
  ...
  chWidth (width, colIndex = -1) {
    ...
    return this.width(width * 1.5, colIndex);
  }
  ...
  width (width, colIndex = -1) {
    if (colIndex < 0) {
      ...
      let fsz = this.curCell.s.font && this.curCell.s.font.sz || 10;
      col.wpx = this.width2wpx(width, fsz);
    } else {
      let col = this['!cols'][colIndex];
      if (!col) {
        col = this['!cols'][colIndex] = {};
      ...
      let fsz = cellRef && cellRef.s.font && cellRef.s.font.sz || 10;
      col.wpx = this.width2wpx(width, fsz);
    }
    return this;
  }
